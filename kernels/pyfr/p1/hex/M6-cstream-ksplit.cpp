...
gimmik_mm_24x24(const double* __restrict__ b, double* __restrict__ c)
{
    ...
    if (threadIdx.y == 0)
    {
        ...
        dotp = (-1.366025403784439*bv[2] + -0.3660254037844385*bv[6]);
        cv[4] = dotp;
        dotp = make_zero();
        csub[0][9][threadIdx.x] = dotp;
        dotp = (0.3660254037844385*bv[2] + 1.366025403784439*bv[6]);
        cv[5] = dotp;
        dotp = make_zero();
        csub[0][11][threadIdx.x] = dotp;
        bv[3] = __ldcg(b + i + 6*ldb); 
        bv[7] = __ldcg(b + i + 14*ldb); 
        dotp = (-1.366025403784439*bv[3] + -0.3660254037844385*bv[7]);
        cv[6] = dotp;
        dotp = make_zero();
        csub[0][13][threadIdx.x] = dotp;
        dotp = (0.3660254037844385*bv[3] + 1.366025403784439*bv[7]);
        cv[7] = dotp;
        dotp = make_zero();
        csub[0][15][threadIdx.x] = dotp;
        bv[0] = __ldcg(b + i + 0*ldb); 
        bv[10] = __ldcg(b + i + 20*ldb); 
        dotp = (-1.366025403784439*bv[0] + -0.3660254037844385*bv[10]);
        cv[8] = dotp;
        dotp = make_zero();
        csub[0][17][threadIdx.x] = dotp;
        bv[1] = __ldcg(b + i + 2*ldb); 
        bv[11] = __ldcg(b + i + 22*ldb); 
        dotp = (-1.366025403784439*bv[1] + -0.3660254037844385*bv[11]);
        cv[9] = dotp;
        dotp = make_zero();
        csub[0][19][threadIdx.x] = dotp;
        dotp = (0.3660254037844385*bv[0] + 1.366025403784439*bv[10]);
        cv[10] = dotp;
        dotp = make_zero();
        csub[0][21][threadIdx.x] = dotp;
        dotp = (0.3660254037844385*bv[1] + 1.366025403784439*bv[11]);
        cv[11] = dotp;
        dotp = make_zero();
